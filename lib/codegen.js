...
const generateDefinitionFile = (config, def, def_name, index) => new Promise((resolve, reject) => {
  fs.readFile(path.join(config.root, config.file_name), 'utf8', (err, data) => {
    ...
    // replace extended parameters keys as it plays bad with handlebars
    // x-nullable
    // const props = Object.keys(def.properties).map((prop_key) => {
    //   delete Object.assign(def.properties[prop_key], {['nullable']: def.properties[prop_key]['x-nullable'] })['x-nullable'];
    // });
