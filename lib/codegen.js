 ...
 * @returns {Promise}
 */
const generateDefinitionFile = (config, def, def_name) => new Promise((resolve, reject) => {
  fs.readFile(path.join(config.root, config.file_name), 'utf8', (err, data) => {
    ...
    const new_filename = config.file_name.replace('---', def_name);
...
const generateDefinitionFiles = config => new Promise((resolve, reject) => {
  ...
  _.each(config.data.swagger.definitions, (def, def_name) => {
    const definition_name = def_name.toLowerCase();
    if (files[definition_name] === undefined) {
      files[definition_name] = [];
    }
    files[definition_name].push({
      definition_name,
      def
    });
  });
  // console.log(files)
  Promise.all(
    _.map(files, (definition, definition_name) => generateDefinitionFile(config, definition, definition_name))
