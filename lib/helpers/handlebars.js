...
/**
 * Converts a string to its snake-cased version.
 */
 ...
 */
Handlebars.registerHelper('inside', (lvalue, rarray, options) => {
  if(!rarray){
    return options.inverse(this);
  }
  return rarray.includes(lvalue) ? options.fn(this) : options.inverse(this);
});
...
/**
 * Object keys array litteral
 */
Handlebars.registerHelper('keys', function(context) {
  if (!context) {
    return '[]';
  }
  return JSON.stringify(Object.keys(context)).split("\"").join( "'");
});
